...
BYTE gTxLength = 0; //EXTERNED to MotionControl.h
BYTE gUSBTxBuf[UART_TX_BUFFER_SIZE]; //EXTERNED to MotionControl.h
BYTE gCmdLength = 0;	//the total number of bytes in the command
BOOL gBNewCmd = TRUE;
BYTE gBufIndex = 0;
...
static void BulkOut(U8 bEP, U8 bEPStatus)
{
    ...
    if( gBNewCmd ) //starting a new command
    {
      //the first byte specifies the size of the command in bytes
      gCmdLength = abBulkBuf[0];
      //note that we are now in the process of receiving a command
      gBNewCmd = FALSE;
    }
    if ( iLen >= gCmdLength ) //finished receiving command
    {  	
      //transfer the command packet to the motion control subsystem
      parse_command( (BYTE*)abBulkBuf, gCmdLength );
      //reset the index
      bufferDumpFromFront(&uartRxBuffer,gCmdLength);
      iLen = 0;  //necessary?
      //the next data to arrive should be a new command
      gBNewCmd = TRUE;
    }
}
...
int VCOM_putchar(int c)
{
	...
	//if there is no space the character will be lost, unfortunately
...
void usbSetup(void)
{
	...
	// not using because it introduces jitter into the timer1 interrupts
	...
	install_irq(USB_INT,(void *)USBIntHandler);  //setup IRQ for USB
	CRITICAL_SECTION_END;
	
	processorEnableInt(CPSR_MASK_IRQ);  //enable IRQ and FIQ interrupts
